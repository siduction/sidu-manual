    ...
    def handle(self, application, documentRoot, startResponse):
        ...
        if not "PATH_INFO" in self._environ:
            ...
            if url.startswith("/static"):
                rc = self.handleStaticFiles(url, documentRoot, startResponse)
            else:
                ...
                if info == None:
                    ...
                    if isinstance(rc, HttpResponse):
                        ...
                        raise Exception("unexpected result type: " + dumpObj(rc))
                    startResponse.__call__(status, headers)
        return self.makeList(rc)
      
    def makeList(self, rc):
        '''Ensures that the result is a list.
        @param rc: value to ensure
        @return: a list
        '''
        if rc == None:
            rc = [""]
            ...
            ix = len(rc) - 1
            # Unicode kills writing to the socket:
            while ix >= 0:
                if type(rc[ix]) == unicode:
                    rc[ix] = rc[ix].encode("utf-8")
                ix -= 1
        return rc
