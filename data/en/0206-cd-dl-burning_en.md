...
The **.gpg** files are the signature files used to validate checksum files (.md5, .sha256) for changes. 
...
### md5sum and integrity check

An md5sum is the checksum of a file and is used to check the integrity of the associated file. It compares the current file's md5sum with an already known sum. This way, it can be determined if the file has been modified or damaged, which is always advisable for files downloaded from the web and saves a lot of time for debugging.

The downloaded file is undamaged if its md5sum matches the sum in the MD5 file. On Linux, you can get the md5sum of a file with:

    $ md5sum file_to_be_checked

This takes some time and the sum is then printed onto console. It can be compared manually with the sum in the corresponding *.md5 file. The md5 file can be opened in a text editor for this purpose. With the md5summer (486 KB), the md5sum can also be checked in Windows.

On Linux, it is more convenient to use the following command, executed in the directory in which both the ISO file and the ISO.MD5 file are located:

    $ md5sum -c to_check_file.md5

Depending on whether the checksums match, you will get a message from the program:

    md5-sum: file_to_be_checked: ok

or

    siduction-name.iso: failure md5sum: WARNING: 1 of 1 computed checksum did NOT match.
...
> siduction, as a Linux LIVE DVD/CD, is heavily compressed. For this reason, special attention must be paid to the burning method of the image. Please use high quality media, burning in DAO mode (Disk-At-Once) and not faster than eight times (8x). However, we recommend to put the image on a USB stick or SD memory card, provided the hardware supports booting from USB. For this purpose, we recommend the tool Edger or the command line tool dd. Instructions are provided in the manual under Installation Options.


Of course, the DVD can also be burned on Windows. The downloaded file must be burned as an ISO image file. If Winrar (or any other archiving program) is linked to ISO files, this program could see the ISO file as an archive file. The ISO file must be used to burn a DVD.

There are several good options to burn ISO files on Windows.  
...
### Burning a DVD with Linux

> **IMPORTANT INFORMATION:**  
> siduction, as Linux LIVE DVD/CD, is heavily compressed. For this reason, special attention must be paid to the burning method of the ISO image. Please use high quality media, burning in DAO mode (Disk-At-Once) and not faster than eight times (8x). However, we recommend to put the image on a USB stick or SD memory card, provided the hardware supports booting from USB. For this purpose, we recommend the tool Edger or the command line tool dd. Instructions for this can be found in the manual under installation options.


If you already have Linux on your computer, you can create the DVD with any installed burning program. On siduction, K3b is the default burning program. There you have to click the menu item "Tools" -> "Burn ISO image...", select the ISO file to burn (e.g. siduction-18.3.0-patience-kde-amd64-201805132121.iso), and set the burning mode DAO (Disk-At-Once).

K3b first computes the MD5 sum of the ISO file (takes a moment). If the displayed checksum matches the given string of the MD5 file located in the same folder (e.g. siduction-Name.iso.md5), the download was successful and the file can be burned by clicking on "Start".

If you click on the computed checksum, an icon will appear next to it. If you click on it again and insert the checksum from the MD5 file into the field, the two checksums will be compared.
...
See also [Installing to USB stick/SSD from another system](0303-hd-ins-opts-oos_en.md#installation-on-usb-stick---memory-card) (Linux, MS Windows, Mac OS X).
