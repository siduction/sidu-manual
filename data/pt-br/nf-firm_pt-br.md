...
Como a ISO do siduction contém apenas software livre, você provavelmente sentirá necessidade de adicionar repositórios com aplicações não livres (`contrib non-free` ) às suas `/etc/apt/sources.list.d/debian.list`  :

~~~  
mcedit /etc/apt/sources.list.d/debian.list  
~~~

As linhas que você inserir devem se parecer com estas, dependendo dos espelhos ('mirrors') que você escolheu (refer to  [http://www.debian.org/mirrors/">Listagem dos servidores Debian e status atual dos espelhos ('mirrors')](http://www.debian.org/mirrors/)):
...
Para instalar firmware non-free firmware, (é supost que `contrib non-free`  foi adicionado às suas sources):
...
O firmware necessário pode ser determinado pelo script recém-disponibilizado fw-detect (empacotado em siduction-scripts).

~~~  
$ fw-detect  
~~~

A saída de fw-detect detalha os comandos necessários para instalar e ativar o firmware:
...
~~~
 ...
 `Para instalar firmare pelos repositórios git, você precisa de:` 

~~~  
apt-get install git-core  
~~~

### pacotes .deb com firmware não-livre para pendrives

Se você tiver de preparar pacotes .deb de firmware e transferi-los para outro computador usando uma pendrive ou algum outro dispositivo USB, você pode baixá-los tanto como arquivos .zip quanto como arquivos tar.gz em  [http://cdimage.debian.org/cdimage/unofficial/non-free/firmware/sid/current/](http://cdimage.debian.org/cdimage/unofficial/non-free/firmware/sid/current/) . Depois, extraia-os para uma pasta com o nome de firmware (devem aparecer vários arquivos .deb). 

A seguir, baixe  [firmware-linux-free*.deb](http://packages.debian.org/sid/firmware-linux-free)  e adicione-o a todos os outros debs na pasta. Daí, transfira tudo para a pendrive e desta para o computador que precisa do firmware não livre e:
...
+  **`audio`**   
    + **`Cirrus Logic (Sound Fusion) CS4280/CS461x/CS462x/CS463x `**  
        ...
        siga cuidadosamente o howto descrito no bugreport, MD5: 6510e1759d85b0627d36367febbb3a6a  
    + **`EMI 2|6 `**  
        emi26  
...
+  **`ieee802.11`**   
    ...
    + **``**  
        ...
        1. Para a maior parte das placas b43 802.11b/g já não deve ser necessário o firmware non-free. 2. No caso de estarem os dois firmwares instalados o non-free é preferencialment usado.  
    + **`Broadcom b43legacy `**  
        b43legacy  
...
+  **`tv`**   
    ...
    + **`various full featured DVB `**  
        ...
        a maioria das placas mais baratas não necessita disto  
    + **`IVTV `**  
...
+  **`vga`**   
    ...
    + **`nVidia `**  
        nv  
        aceleração 3D ainda não é possível com drivers livres  
