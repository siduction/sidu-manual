...
<!--Daher könnte es vom Anwender gewünscht sein, `contrib non-free`  in den Dateien `/etc/apt/sources.list.d/debian.list und /etc/apt/sources.list.d/siduction.list`  zu ergänzen:
...
~~~
 ...
 `Um Firmware von git-Repos installieren zu können, muss git-core installiert werden:` 
